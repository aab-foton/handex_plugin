...
figma.ui.onmessage = async (msg) => {
    ...
    const gap = (_a = msg.gap) !== null && _a !== void 0 ? _a : 80; // espaçamento entre os frames clonados
    try {
        ...
        figma.ui.postMessage({ type: 'error', text: `Erro: ${(_b = err === null || err === void 0 ? void 0 : err.message) !== null && _b !== void 0 ? _b : 'Desconhecido'}` });
    }
};
// ────────────────────────────────────────────────────────────
 ...
 */
async function generateBreakpoint(sourceFrame, targetWidth, gap) {
 ...
 */
function applyResponsiveness(node, originalParentWidth, newParentWidth, scaleFactor) {
    ...
    for (const child of container.children) {
        ...
        if (child.type === 'FRAME' && child.layoutMode !== 'NONE') {
            handleAutoLayoutFrame(child, originalParentWidth, newParentWidth, scaleFactor);
            // ── Caso B: filho não tem Auto Layout (posicionamento livre) ──
        }
        else if ('constraints' in child) {
            handleConstrainedNode(child, originalParentWidth, newParentWidth, scaleFactor);
        }
 ...
 */
function handleAutoLayoutFrame(frame, originalParentWidth, newParentWidth, scaleFactor) {
    ...
    if (frame.layoutSizingHorizontal === 'FILL') {
        ...
        catch (_) { /* ignora erros de resize */
        }
    }
 ...
 */
function handleConstrainedNode(node, originalParentWidth, newParentWidth, _scaleFactor) {
    ...
    switch (h) {
        ...
        case 'MAX': // RIGHT – mantém distância da borda direita
            try {
                node.x = newParentWidth - currentWidth - distFromRight;
            }
            catch (_) { }
            break;
        case 'CENTER': { // Mantém centralizado
            ...
            try {
                node.x = Math.round(newCenter - currentWidth / 2);
            }
            catch (_) { }
            break;
        }
        case 'SCALE': { // Escala proporcionalmente (X e width)
            ...
            try {
                node.x = Math.round(xRatio * newParentWidth);
                node.resize(Math.round(widthRatio * newParentWidth), node.height);
            }
            catch (_) { }
            break;
        }
        case 'STRETCH': { // Estica entre margens esquerda e direita
            ...
            try {
                node.resize(Math.max(newWidth, 1), node.height);
            }
            catch (_) { }
            break;
        }
 ...
 */
function getRightEdge(sourceFrame, gap) {
    ...
    const maxRight = Math.max(sourceFrame.x + sourceFrame.width, ...sameLine.map(f => f.x + f.width));
    return maxRight + gap;
}
/**
 ...
 */
function estimateChildrenTotalWidth(frame) {
    let total = 0;
    for (const child of frame.children) {
        if ('width' in child)
            total += child.width;
 ...
 */
function getOriginalChildWidth(child, scaleFactor) {
    if (!('width' in child))
        return 0;
    return child.width / scaleFactor;
}
