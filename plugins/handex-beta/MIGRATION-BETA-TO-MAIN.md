...
**Bug real — `fix-tabordem-varredura-aninhada` (2026-08-24).** Testado no Figma real pela primeira vez: "Gerar Automaticamente" retornava "Nenhum elemento interativo encontrado" numa área que continha Button/Tab Group reais (confirmado pelo designer — o mesmo scan de "Elementos e Imagens" reconhecia esses componentes com alta confiança). Causa: em `generate-tab-order-from-layers` (`code.js`), a função `_walk` fazia `continue` incondicional depois de avaliar QUALQUER `INSTANCE`/`COMPONENT`, interativo ou não — interrompendo a descida em qualquer container real (Card, Section, wrapper de filtro — tudo isso costuma ser `INSTANCE` de componente DSC) antes de alcançar botões/abas/inputs aninhados dentro dele. Corrigido: a descida só é interrompida quando o PRÓPRIO nó já foi capturado como candidato interativo (`isInteractiveMatch`); em qualquer outro caso (container não interativo, ou DSC sem mapeamento/`isUnmapped`) a varredura continua recursivamente. Esse bug provavelmente afetava toda tela real com hierarquia de componentes (praticamente qualquer tela DSC não-trivial) — telas de teste anteriores com componentes soltos direto no frame não o expunham. **Arquivo alterado:** `src/plugin/code.js` (`generate-tab-order-from-layers`). **Risco de migração:** baixo, correção pontual e isolada — mas herda o risco médio já documentado da seção-mãe (fluxo assíncrono de clonagem ainda não 100% validado em produção).
...
**Revisão 2 — `fix-accordion-resumo-lote` (2026-08-24).** Mesmo modal, problema irmão: a lista de grupos elegíveis ao lote (`#a11y-batch-summary-groups` — "2 Accordion", "11 Button", "44 Decorative"...) é puramente informativa (nenhuma ação por linha, confirmado pelo designer: "não tenho ação nenhuma nelas") e podia chegar a 9-10 grupos numa tela real, competindo pelo mesmo espaço vertical que o bloco de aviso já resolvido acima. Aplicada a MESMA solução (accordion recolhido, mesmo helper `toggleAccordion`): `modals.html` — novo wrapper `#a11y-batch-summary-groups-block` com cabeçalho clicável (`#a11y-batch-summary-groups-title`) envolvendo o `#a11y-batch-summary-groups` original dentro de um `.accordion-content` que nasce `hidden`. `accessibility.js` (`openA11yBatchSummaryModal`) ganhou o mesmo padrão de reset+contador: título mostra "Componentes detectados (N grupos, M itens)", e o accordion é sempre resetado pra fechado a cada abertura do modal. Nenhuma mudança de critério/agrupamento (`groups`/`groupList`) — só a apresentação. Risco de migração: baixo, aditivo, mesma dependência de `toggleAccordion` já usada nas outras duas revisões deste modal.
...
## `a11y-nao-documentados`

**O que é.** Novo accordion "Não Documentados (N)" dentro de cada Área Marcada, na listagem agrupada da aba Acessibilidade (`renderA11yGroupedList` → `_a11yAreaAccordionEl`), ao lado dos 4 accordions de categoria já existentes (Elementos e Imagens/Estrutura/Título/Decorativo) e da seção "Ordem de Tabulação". Lista todo elemento que o SCAN de a11y já identificou dentro daquela área mas que ainda não virou spec nenhuma — hoje esses itens só apareciam, de forma fragmentada e transitória, dentro do modal "Gerar Handoff Automatizado" (bloco de grupos elegíveis + bloco de aviso "Possíveis títulos sem token DSC"), e desapareciam da tela assim que o modal fechava sem o designer confirmar o lote.

**Decisões de escopo (confirmadas com o designer antes de implementar, não assumidas).**
1. Junta DUAS fontes que hoje vivem em variáveis separadas: (a) candidatos com `dscComponentMatch` válido — com ou sem `a11yCategory` mapeada, incluindo `isUnmapped` ("Outro") — que nunca foram confirmados nem pelo fluxo manual nem pelo lote; e (b) candidatos `needsA11yTokenReview` (texto sem token DSC vinculado, feature `a11y-aviso-titulo-sem-token`).
2. Ao criar uma spec a partir de um item da lista, ele desaparece dali e passa a contar no accordion da categoria real — **sem campo novo de "revisado" no schema**. A lista é recalculada dinamicamente a cada render (`_collectA11yUndocumentedForArea`), exatamente como os 4 accordions de categoria já fazem hoje (eles também não guardam estado próprio, só filtram `a11ySpecs` vivo por `a11yType`).
3. Não aparece na Ficha de Handoff final — é só ferramenta de trabalho da UI do plugin (nenhuma alteração em `handoff.js`/`generateA11yFicha`).
4. Cada item tem uma ação "Criar spec" que reaproveita o MESMO formulário manual (`openA11yModal`) que o botão "+ Nova spec" do card da área abre, pré-preenchido com a sugestão do scan (categoria/subtipo quando há `dscComponentMatch`; categoria "Informações Adicionais" — sem subtipo específico — quando é um item `needsA11yTokenReview`).

**Lacuna de dados descoberta durante a investigação (não estava no pedido original, mas bloqueava a implementação correta).** `frame.a11yDetections`/`window._a11yLooseDetections`/`window._a11yTokenReviewCandidates` (variáveis pré-existentes de `a11y-deteccao-automatica`) são **sobrescritas a cada scan de área** — não acumulam por área nem guardam qual área originou cada item. Como o designer tipicamente marca várias áreas antes de revisar pendências, usar essas variáveis direto faria o accordion de uma área "roubar" ou perder os candidatos de outra. Resolvido acrescentando dois dicionários novos, escopados por `areaId`, preenchidos em paralelo às variáveis antigas (que continuam existindo e alimentando o lote exatamente como antes):
```js
window._a11yDetectionsByArea[areaId] = detections;
window._a11yTokenReviewByArea[areaId] = tokenReviewCandidates;
```
Preenchidos dentro de `handleA11yPostAreaDetectionResult`, a partir de `window._a11yPendingDetectionArea.areaId` (já existia, usado só pra pré-selecionar o `<select>` do modal de lote). Não persiste entre sessões do plugin — mesma natureza efêmera do resto da Detecção Automática (`window._a11yLooseDetections` também não persiste).

**Correspondência "já documentado" (como o cruzamento scan × specs foi resolvido).** Por `targetNodeId`: toda spec de a11y nasce com `targetNodeId` = `nodeId` real do elemento no canvas (`opts.targetNodeId`, tanto no fluxo manual quanto no lote — confirmado lendo `confirmA11ySpec`/`confirmA11yBatchGenerate`, ambos usam o mesmo campo). `_collectA11yUndocumentedForArea(areaId)` monta um `Set` com os `targetNodeId` de todas as specs de `a11ySpecs` daquela área e filtra os candidatos do scan cujo `nodeId` não está nesse `Set` — mesmo padrão de derivação "sem estado próprio" já usado pelos 4 accordions de categoria.

**Formulário aberto por tipo de item.**
- `dscComponentMatch.isUnmapped === true` → categoria "Elementos e Imagens", sem `presetComponente` (não há como pré-preencher o campo de texto livre "Componente" sem duplicar a lógica de `prefillA11yComponentName`; o designer confirma manualmente).
- `a11yCategory === 'titulo'` → categoria "Nível de Título", com `presetTituloNivel = dscComponentMatch.suggestedLevel` (novo parâmetro de `openA11yModal`).
- `a11yCategory === 'decorativo'` → categoria "Elemento Decorativo", default "gerais" (`openA11yModal` já assume esse default sozinho).
- `a11yCategory === 'estrutura'` → categoria "Estrutura da Página", com `presetEstruturaTipo` derivado de `_inferA11yEstruturaTipoFromContainingFrame` (função já existente, reaproveitada por leitura — mesma usada no lote) — novo parâmetro de `openA11yModal` que também força `variacao = 'marco de navegacao'` em vez do default `'idiomas'`.
- Qualquer outro `a11yCategory` (nome de componente do catálogo de 16) → categoria "Elementos e Imagens", com `presetComponente = a11yCategory` (parâmetro que já existia, introduzido por `a11y-deteccao-automatica`).
- `kind === 'tokenReview'` (sem `dscComponentMatch`) → categoria "Informações Adicionais" (decisão tomada após inspecionar o formulário manual: é a categoria mais plausível pra um texto solto sem função de título/componente clara — confirmada antes de implementar, não é suposição), sem subtipo específico (fica no default `'handoffs'` que `openA11yModal` já aplica).

Em todos os casos, `openA11yModal` recebeu um novo parâmetro `pendingTargetNodeId`, gravado em `modal.dataset.pendingTargetNodeId` e lido por `confirmA11ySpec` (`opts.targetNodeId = pendingTargetNodeId` quando não é edição) — mesmo raciocínio de `editingSpec.targetNodeId` já usado no fluxo de editar spec, só que para criação nova a partir de um nó já conhecido em vez da seleção atual do canvas.

**Checagem de vínculo da lib "Design Acessível".** A ação "Criar spec" passa pela MESMA checagem (`check-a11y-library`) que o botão "+" normal usa — decisão de manter consistência (se a lib não estiver acessível, o aviso/passo a passo deve ser igual em qualquer caminho de criação). Como a resposta bem-sucedida da checagem (`a11y-library-status`, `messages.js`) sempre abria o seletor de categoria (`_openA11yCategoryPickerModalNow`), e aqui a categoria já é conhecida (perguntar de novo seria redundante), foi adicionado um desvio opcional: `window._a11yLibCheckOnSuccess`, um callback que, quando setado, substitui a abertura do seletor de categoria. Limpo em três pontos pra não vazar entre fluxos (`openA11yCategoryPickerModal` no início do fluxo normal "+", no branch "não vinculado" da resposta, e consumido uma única vez — `null` logo após a leitura — no branch "vinculado").

**Componentes visuais novos (`accessibility.js`).**
- `_a11yUndocumentedAccordionEl(uid, areaId, entries)` — mesmo padrão estrutural de `_tabOrderSectionHtml`/`_a11yCategoryAccordionEl` (header clicável com chevron + corpo `accordion-content`/`hidden`), mas paleta âmbar (mesma da revisão "baixa confiança"/"Verificar" já usada em specs do lote) em vez da paleta neutra dos 4 accordions de categoria — decisão de design tomada sem pedido explícito: reforça visualmente que é uma pendência, não uma spec já concluída. Some da área por completo (retorna `''`) quando não há nenhum item pendente, mesmo critério dos 4 accordions de categoria.
- `_a11yUndocumentedItemHtml(areaId, entry)` — card por item: nome da camada, rótulo da sugestão (`_a11yUndocumentedItemLabel`), botão "Focar no canvas" (`focusNode`, mesmo padrão de toda a aba) e botão "Criar spec" (`openA11yFormFromUndocumented`). O objeto `item` completo viaja serializado (`encodeURIComponent(JSON.stringify(item))`) dentro do atributo `onclick`, decodificado de volta no handler — mesma necessidade que o lote não tinha (ali os itens ficam em `window._a11yBatchDetections`, aqui cada botão precisa carregar seu próprio item individualmente).
- Estado de expansão em `window._a11yExpandedUndocumentedIds` (Set próprio, não compartilha com `_a11yExpandedAreaIds`/`_a11yExpandedTabOrderIds`) — mesmo padrão de isolamento já usado por `_tabOrderSectionHtml`.
- `_a11ySetAllSubaccordions` (função nova desta mesma sessão, ver `a11y-expandir-recolher-todos` abaixo, aplicada em paralelo por outro agente) precisou de um terceiro prefixo de id reconhecido (`undoc-body-`/`undoc-chevron-`, ao lado de `body-`/`tab-order-body-`) pra também expandir/recolher o novo accordion — checar essa função no momento da migração se `a11y-expandir-recolher-todos` for portada em separado.

**Arquivos alterados:** `src/plugin/modules/accessibility.js` (`_collectA11yUndocumentedForArea`, `_a11yUndocumentedAccordionEl`/`_a11yUndocumentedItemHtml`/`_a11yUndocumentedItemLabel`, `toggleA11yUndocumentedAccordion`, `openA11yFormFromUndocumented`, extensão de `openA11yModal`/`confirmA11ySpec`/`openA11yCategoryPickerModal`/`_a11ySetAllSubaccordions`, acumulação por área em `handleA11yPostAreaDetectionResult`, chamada em `_a11yAreaAccordionEl`), `src/plugin/modules/messages.js` (desvio `window._a11yLibCheckOnSuccess` no handler `a11y-library-status`). Nenhuma alteração em `code.js`, `modals.html` nem `handoff.js`.

**Risco de migração:** baixo-médio. Aditivo em quase todos os pontos (novo accordion, novos parâmetros opcionais de `openA11yModal`, novos dicionários por área). Único ponto de atenção real: os dois novos dicionários (`window._a11yDetectionsByArea`/`_a11yTokenReviewByArea`) duplicam em memória o que `window._a11yLooseDetections`/`_a11yTokenReviewCandidates` já guardavam — se uma migração futura decidir persistir candidatos entre sessões (hoje não persiste, decisão consciente), os quatro pontos de escrita precisam ser revisitados juntos. Depende de `a11y-deteccao-automatica` (item 4, onde `_collectA11yDetections`/`dscComponentMatch`/`_filterA11yBatchEligible` nasceram), `a11y-aviso-titulo-sem-token` (candidatos `needsA11yTokenReview`), `a11y-outro-componentes-sem-match` (`isUnmapped`) e `a11y-mapeamento-header-footer-logo` (`_inferA11yEstruturaTipoFromContainingFrame`) já migrados antes.

---

## `a11y-rescan-lote`

**O que é.** Botão "Reescanear" (ícone de refresh) no cabeçalho do modal "Gerar Handoff Automatizado" (`#a11y-batch-summary-modal`). Pedido do designer ao notar que, ao vincular um Text Style do DSC num item do bloco "Possíveis títulos sem token DSC" (pra fazer aquele texto virar sugestão real), não havia como ver o resultado atualizado sem fechar o modal inteiro e refazer o fluxo de detecção da área do zero.

**Decisão de escopo (perguntada e confirmada antes de implementar).** O designer optou pela opção mais simples entre três propostas: um botão de reescanear DENTRO do próprio modal, que roda o mesmo scan de novo e reconstrói as duas listas (grupos elegíveis + aviso de token) sem fechar o modal — não um mecanismo de merge/diff que preserve edições feitas dentro do modal (não havia nenhuma edição a preservar: o modal é só leitura + um select de área).

**Implementação.** `rescanA11yBatchArea()` (`accessibility.js`) reaproveita `runA11yPostAreaDetection()` (já existente, `a11y-deteccao-automatica`) sem nenhuma duplicação de lógica de scan — ela já lê `window._a11yPendingDetectionArea`, que **continua vivo** enquanto o modal de resumo está aberto (decisão pré-existente, documentada no comentário de `handleA11yPostAreaDetectionResult`: `closeA11yPostAreaDetectModal` — que zeraria essa variável — não é chamado nesse ponto do fluxo, só um `closeModal` direto no modal de loading). O fluxo de resposta padrão (`scan-result` → `handleA11yPostAreaDetectionResult` → `openA11yBatchSummaryModal`) já é idempotente e reconstrói ambas as listas do zero mesmo com o modal já visível — não precisou de nenhum código novo de merge. Dois ajustes necessários:
1. Spinner no botão (`btn-a11y-batch-rescan`, classe `animate-spin` + `disabled`) enquanto a resposta do scan não chega — sinalizado por `window._a11yBatchRescanBtnPending`, lido e restaurado no início de `handleA11yPostAreaDetectionResult` (a resposta é assíncrona, viaja até `code.js` e volta, então não dá pra restaurar sincronamente dentro da própria função que dispara).
2. Caso de borda: se depois do reescaneio não sobrar nenhum candidato elegível nem de aviso (ex: o único item pendente foi corrigido e não há mais nada a revisar), o branch existente de "nada encontrado" só fechava o modal de loading (`closeA11yPostAreaDetectModal`) — correto no fluxo normal (o modal de resumo nunca chegou a abrir), mas incorreto num reescaneio (o modal de resumo JÁ está aberto com dados agora obsoletos). Adicionado `closeModal('a11y-batch-summary-modal')` nesse branch, ao lado do já existente.

**Arquivos alterados:** `src/plugin/modules/accessibility.js` (`rescanA11yBatchArea`, ajustes em `handleA11yPostAreaDetectionResult`), `src/plugin/views/modals.html` (botão no cabeçalho de `#a11y-batch-summary-modal`).

**Risco de migração:** baixo — aditivo, reaproveita 100% da lógica de scan já existente (`runA11yPostAreaDetection`/`handleA11yPostAreaDetectionResult`/`openA11yBatchSummaryModal`), sem novo estado persistido. Depende de `a11y-deteccao-automatica` (item 4) já migrado.
