...
figma.clientStorage.getAsync('handoffData').then(savedState => {
...
figma.ui.onmessage = async (msg) => {
  if (msg.type === "create-handoff") {
    try {
      ...
      // Helpers
      function createText(text, size = 14, weight = "Regular", color = { r: 0.12, g: 0.16, b: 0.23 }, stretch = true) {
      ...
      function createFrame(direction = "VERTICAL", padding = 0, spacing = 0, fill = null) {
        ...
        // Defaults to Hug in both axes
        if (direction === "VERTICAL") {
          f.primaryAxisSizingMode = "AUTO"; // Height: Hug
          f.counterAxisSizingMode = "AUTO"; // Width: Hug
        } else {
          f.primaryAxisSizingMode = "AUTO"; // Width: Hug
          f.counterAxisSizingMode = "AUTO"; // Height: Hug
        }
      ...
      function createRow(label, value, isLink = false, url = "") {
        const row = createFrame("VERTICAL", 0, 4);
        row.layoutAlign = "STRETCH";
        row.appendChild(createText(label, 12, "Bold", { r: 0.39, g: 0.45, b: 0.55 }));
      ...
      function createInputBox(value, isTextArea = false) {
        const box = createFrame("HORIZONTAL", 12, 0, { r: 1, g: 1, b: 1 });
        box.strokes = [{ type: "SOLID", color: { r: 0.8, g: 0.85, b: 0.9 } }];
        box.cornerRadius = 4;
        box.layoutAlign = "STRETCH";
        box.paddingTop = 8;
        box.paddingBottom = isTextArea ? 48 : 8;
        const text = createText(value || "", 14, "Regular", { r: 0.4, g: 0.45, b: 0.5 }, false);
        text.layoutGrow = 1;
      ...
      function createCard(titleText) {
        const card = createFrame("VERTICAL", 16, 16, { r: 0.99, g: 0.99, b: 0.99 });
        card.strokes = [{ type: "SOLID", color: { r: 0.9, g: 0.92, b: 0.95 } }];
        card.cornerRadius = 4;
        card.layoutAlign = "STRETCH";
        const title = createText(titleText, 14, "Bold", { r: 0, g: 0.35, b: 0.79 });
      ...
      function createContactField(label, value) {
        const field = createFrame("VERTICAL", 0, 4);
        field.layoutGrow = 1;
        field.layoutAlign = "STRETCH";
        field.appendChild(createText(label, 12, "Bold", { r: 0.2, g: 0.2, b: 0.2 }));
        const inputBox = createInputBox(value || "-");
        inputBox.layoutAlign = "STRETCH";
        field.appendChild(inputBox);
        
        if (value && value.includes('@')) {
          const linkRow = createFrame("HORIZONTAL", 0, 4);
          linkRow.layoutAlign = "STRETCH";
          linkRow.counterAxisAlignItems = "CENTER";
          const icon = createText("↗", 12, "Bold", { r: 0, g: 0.35, b: 0.79 }, false);
          const link = createText("Entrar em contato", 10, "Regular", { r: 0, g: 0.35, b: 0.79 }, false);
      ...
      function drawCaixaLogo(parent) {
        const svgString = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 205.51265 46.553631">
        ...
        try {
          const logoNode = figma.createNodeFromSvg(svgString);
          logoNode.name = "LOGO CAIXA";
          // Ajustar tamanho para o cabeçalho (proporção original ~205/46)
          const targetHeight = 24;
          const targetWidth = (205.5 / 46.5) * targetHeight;
          logoNode.resize(targetWidth, targetHeight);
          parent.appendChild(logoNode);
        } catch (e) {
          console.error("Erro ao gerar SVG logo:", e);
          parent.appendChild(createText("CAIXA", 24, "Bold", { r: 0, g: 0.44, b: 0.73 }, false));
        }
      }

      // MAIN CONTAINER
      const mainContainer = createFrame("HORIZONTAL", 48, 48, { r: 0.97, g: 0.98, b: 0.99 });
      ...
      fichaTecnica.primaryAxisSizingMode = "AUTO"; // Height: Hug
      fichaTecnica.counterAxisSizingMode = "FIXED"; // Width: Fixed 600

      // HEADER (CAIXA)
      const header = createFrame("HORIZONTAL", 24, 0, { r: 1, g: 1, b: 1 }); 
      ...
      drawCaixaLogo(header);

      const headerTitle = createText("Handex - Handoff Expresso", 16, "Bold", { r: 0, g: 0.35, b: 0.79 }, false);
      header.appendChild(headerTitle);
      fichaTecnica.appendChild(header);
      header.layoutAlign = "STRETCH"; // Re-confirm stretch after append
      ...
      // TITLE ROW
      const titleRow = createFrame("HORIZONTAL", 0, 0);
      titleRow.layoutAlign = "STRETCH";
      titleRow.primaryAxisSizingMode = "FIXED"; // Fill width
      titleRow.primaryAxisAlignItems = "SPACE_BETWEEN";
      titleRow.counterAxisAlignItems = "CENTER";
      
      const mainTitle = createText("Ficha de projeto - " + ((data.step1 && data.step1.fluxo) || "HANDEX"), 16, "Bold", { r: 0.2, g: 0.2, b: 0.2 });
      mainTitle.layoutGrow = 1;

      const pillBadge = createFrame("HORIZONTAL", 12, 4, { r: 0.99, g: 0.96, b: 0.89 });
      pillBadge.cornerRadius = 16;
      pillBadge.strokes = [{ type: "SOLID", color: { r: 0.9, g: 0.8, b: 0.5 } }];
      const statusText = (data.step1 && data.step1.status) || "Em construção";
      const pillBadgeText = createText("🛠 " + statusText, 12, "Bold", { r: 0.6, g: 0.4, b: 0.1 }, false);
      pillBadge.appendChild(pillBadgeText);

      titleRow.appendChild(mainTitle);
      titleRow.appendChild(pillBadge);
      content.appendChild(titleRow);

      // Informações Obrigatórias
      const infoCard = createCard("Informações Obrigatórias");
      infoCard.appendChild(createRow("Título do projeto*", (data.step1 && data.step1.fluxo) || "Nome"));
      infoCard.appendChild(createRow("Objetivo da entrega*", (data.step1 && data.step1.objetivo) || "-", false));
      infoCard.appendChild(createRow("Versão", (data.step1 && data.step1.versao) || "v1.0.0"));
      content.appendChild(infoCard);

      // BRIEFING ESTRATÉGICO
      if (data.briefing && data.briefing.questions && data.briefing.questions.length > 0) {
        const briefingCard = createCard("Briefing Estratégico");
        data.briefing.questions.forEach(q => {
          if (q.answer && q.answer.trim() !== "") {
            const qGroup = createFrame("VERTICAL", 0, 4);
            qGroup.layoutAlign = "STRETCH";
            qGroup.appendChild(createText(`[${q.category || "Geral"}] ${q.question}`, 12, "Bold", { r: 0.2, g: 0.2, b: 0.2 }));
            qGroup.appendChild(createInputBox(q.answer, q.answer.length > 100));
            briefingCard.appendChild(qGroup);
          }
        });
        if (briefingCard.children.length > 1) {
          content.appendChild(briefingCard);
        }
      }

      // Equipe e responsáveis
      const eqCard = createCard("Equipe e responsáveis");
      const team = (data.step3 && data.step3.team) || [];
      if (team.length > 0) {
        for (let i = 0; i < team.length; i += 2) {
          const grid = createFrame("HORIZONTAL", 0, 16);
          grid.layoutAlign = "STRETCH";
          grid.primaryAxisSizingMode = "FIXED"; // Fill width
          grid.appendChild(createContactField(team[i].role, team[i].name));
          if (team[i+1]) {
            grid.appendChild(createContactField(team[i+1].role, team[i+1].name));
          } else {
            const empty = createFrame("VERTICAL", 0, 4);
            empty.layoutGrow = 1;
        ...
        eqCard.appendChild(createText("Nenhum membro listado.", 12, "Regular", { r: 0.6, g: 0.6, b: 0.6 }));
      }
      content.appendChild(eqCard);

      // Links importantes
      const linksCard = createCard("Links importantes");
      const linkGrid = createFrame("HORIZONTAL", 0, 16);
      linkGrid.layoutAlign = "STRETCH";
      linkGrid.primaryAxisSizingMode = "FIXED"; // Fill width
      linkGrid.appendChild(createContactField("Protótipo Interativo", (data.step3 && data.step3.proto_link) || ""));
      linkGrid.appendChild(createContactField("Handoff Acessibilidade", (data.step3 && data.step3.a11y_link) || ""));
      linksCard.appendChild(linkGrid);
      content.appendChild(linksCard);

      if (!data.setup || data.setup.ficha !== false) {
        mainContainer.appendChild(fichaTecnica);
      }

      // --- 2. ESPECIFICAÇÕES DA UI (Colunas Horizontais) ---
      const specs = (data.step2 && data.step2.specs) || { colors: [], typography: [], components: [], icons: [], spacings: [], effects: [], borders: [] };

      function createSpecColumn(title, items, type) {
        if (!items || items.length === 0) return;
        
        const col = createFrame("VERTICAL", 24, 24, { r: 1, g: 1, b: 1 });
        col.name = title;
        col.strokes = [{ type: "SOLID", color: { r: 0.9, g: 0.92, b: 0.95 } }];
        col.cornerRadius = 12;
        col.resize(360, 100); 
        col.primaryAxisSizingMode = "AUTO"; 
        col.counterAxisSizingMode = "FIXED";

        const colTitle = createText(title, 18, "Bold", { r: 0, g: 0.35, b: 0.79 });
        col.appendChild(colTitle);

        // Lista vertical interna para os itens
        const list = createFrame("VERTICAL", 0, 12);
        col.appendChild(list); 
        list.layoutAlign = "STRETCH";

        items.forEach(item => {
          const row = createFrame("HORIZONTAL", 10, 10, { r: 0.98, g: 0.99, b: 1 });
          row.cornerRadius = 8;
          row.strokes = [{ type: "SOLID", color: { r: 0.9, g: 0.92, b: 0.95 } }];
          row.counterAxisAlignItems = "CENTER";
          row.layoutAlign = "STRETCH";
          row.primaryAxisSizingMode = "FIXED"; // This allows layoutAlign: STRETCH to show as "Fill" in UI
          row.resize(col.width - 48, row.height); // Fallback resize
          ...
          if (type === "color") {
            ...
            try {
              ...
              rect.resize(16, 16);
              rect.cornerRadius = 2;
              const img = figma.createImage(item.preview);
              rect.fills = [{ type: 'IMAGE', imageHash: img.hash, scaleMode: 'FIT' }];
              row.appendChild(rect);
            } catch (e) {
              const dot = figma.createEllipse();
              dot.resize(6, 6);
              dot.fills = [{ type: "SOLID", color: { r: 0.8, g: 0.8, b: 0.8 } }];
              row.appendChild(dot);
            }
          } else {
            const dot = figma.createEllipse();
          ...
          const txts = createFrame("VERTICAL", 0, 0);
          txts.layoutGrow = 1;
          txts.appendChild(createText(item.name, 9, "Bold", { r: 0.2, g: 0.2, b: 0.2 }));
          
          let subText = item.value || "";
          if (type === "color") subText = item.hex;
          if (subText && subText.length > 20) subText = subText.substring(0, 17) + "..."; // Truncate long values
          
          if (subText) {
            txts.appendChild(createText(subText, 8, "Regular", { r: 0.5, g: 0.5, b: 0.5 }));
          }
          row.appendChild(txts);

          const badge = createFrame("HORIZONTAL", 3, 1.5, item.isDS ? { r: 0.9, g: 1, b: 0.9 } : { r: 1, g: 0.9, b: 0.9 });
          badge.cornerRadius = 3;
          badge.appendChild(createText(item.isDS ? "DSC" : "Custom", 7, "Bold", item.isDS ? { r: 0, g: 0.5, b: 0.3 } : { r: 0.7, g: 0.2, b: 0.2 }, false));
      ...
      createSpecColumn("Cores Utilizadas", specs.colors, "color");
      createSpecColumn("Tipografia", specs.typography, "typography");
      createSpecColumn("Espaçamentos", specs.spacings, "spacing");
      createSpecColumn("Efeitos", specs.effects, "effect");
      createSpecColumn("Bordas e Raios", specs.borders, "border");
      createSpecColumn("Componentes", specs.components, "component");
      createSpecColumn("Ícones", specs.icons, "icon");

      // 3. ANATOMIA / ESPECIFICAÇÕES TÉCNICAS
      ...
      if (selection.length > 0 && data.setup && (data.setup.espacamentos || data.setup.anatomia || data.setup.instancias)) {
        for (const node of selection) {
          ...
          specsBoard.resize(800, specsBoard.height);
          ...
          if (data.setup.anatomia || data.setup.espacamentos) {
            const layoutSec = createSection("Layout & Posicionamento");
            const grid = createFrame("HORIZONTAL", 0, 16);
            grid.layoutWrap = "WRAP";

            grid.appendChild(createRow("Position", `X: ${Math.round(node.x)}, Y: ${Math.round(node.y)}`));
            grid.appendChild(createRow("Size", `W: ${Math.round(node.width)}, H: ${Math.round(node.height)}`));

            if ('layoutMode' in node && node.layoutMode !== "NONE") {
              grid.appendChild(createRow("Auto Layout", `Dir: ${node.layoutMode}, Spacing: ${node.itemSpacing}`));
              grid.appendChild(createRow("Padding", `T: ${node.paddingTop}, R: ${node.paddingRight}, B: ${node.paddingBottom}, L: ${node.paddingLeft}`));
            }
            if ('cornerRadius' in node && node.cornerRadius !== figma.mixed) {
              grid.appendChild(createRow("Corner Radius", `${node.cornerRadius}px`));
            }
            layoutSec.appendChild(grid);
            specsBoard.appendChild(layoutSec);
          }

          if (data.setup.instancias || data.setup.anatomia) {
            const appearSec = createSection("Aparência");
            const grid = createFrame("HORIZONTAL", 0, 16);
            grid.layoutWrap = "WRAP";

            if ('opacity' in node) grid.appendChild(createRow("Opacity", `${Math.round(node.opacity * 100)}%`));
            ...
            if ('fills' in node && Array.isArray(node.fills)) {
              ...
              if (sf) {
                ...
                grid.appendChild(createRow("Fills", token ? `${token} (${hex})` : hex));
              }
            }
            if ('strokes' in node && Array.isArray(node.strokes)) {
              ...
              if (ss) {
                ...
                grid.appendChild(createRow("Strokes", `${token ? token + ' (' + hex + ')' : hex} (${node.strokeWeight}px)`));
              }
            }

            if (grid.children.length > 0) {
              appearSec.appendChild(grid);
              specsBoard.appendChild(appearSec);
            }
          }
  ...
  if (msg.type === "scan-frame") {
    ...
    function isDS(name) {
      ...
      return lower.includes("[dsc]") || lower.includes("[ dsc]") || lower.includes("dsc/") || lower.includes("ds/");
    }
    ...
    function extractSpecs(n) {
      try {
        ...
        if ('fills' in n && Array.isArray(n.fills)) {
          ...
          const tokenName = getVar(n, 'fills');
        ...
        // Spacings (Gap / Padding)
        if ('layoutMode' in n && n.layoutMode !== "NONE") {
          if (n.itemSpacing > 0) {
            const token = getVar(n, 'itemSpacing');
            const name = token || `${n.itemSpacing}px`;
            addSpec(specs.spacings, name, { name, value: `${n.itemSpacing}px`, isDS: isDS(name), layerName: n.name }, n.id);
          }
          const pt = n.paddingTop || 0, pr = n.paddingRight || 0, pb = n.paddingBottom || 0, pl = n.paddingLeft || 0;
          if (pt + pr + pb + pl > 0) {
            const tT = getVar(n, 'paddingTop'), tR = getVar(n, 'paddingRight'), tB = getVar(n, 'paddingBottom'), tL = getVar(n, 'paddingLeft');
            const val = `${pt}, ${pr}, ${pb}, ${pl}`;
            const name = (tT || tR || tB || tL) ? `Padding (${[tT, tR, tB, tL].filter(x => x).join('/')})` : `Padding ${val}`;
            addSpec(specs.spacings, name, { name, value: val, isDS: isDS(name), layerName: n.name }, n.id);
          }
        }

        // Borders & Radius
        if ('cornerRadius' in n && n.cornerRadius !== figma.mixed && n.cornerRadius > 0) {
          const token = getVar(n, 'cornerRadius');
          const name = token || `Radius ${n.cornerRadius}px`;
          addSpec(specs.borders, name, { name, value: `${n.cornerRadius}px`, isDS: isDS(name), layerName: n.name }, n.id);
        }
        if ('strokes' in n && Array.isArray(n.strokes) && n.strokes.length > 0) {
          const ss = n.strokes.find(s => s.type === "SOLID");
          if (ss) {
            const hex = rgbToHex(ss.color.r, ss.color.g, ss.color.b).toUpperCase();
            const token = getVar(n, 'strokes');
            const name = token || `Stroke ${hex}`;
            addSpec(specs.borders, name, { name, value: `${hex} (${n.strokeWeight}px)`, isDS: isDS(name), layerName: n.name }, n.id);
          }
        }

        // Effects (Shadows)
        if ('effects' in n && Array.isArray(n.effects) && n.effects.length > 0) {
          const visibleEffects = n.effects.filter(e => e.visible);
          if (visibleEffects.length > 0) {
            const token = getVar(n, 'effects');
            visibleEffects.forEach(eff => {
              const name = token || `${eff.type} ${eff.radius}px`;
              addSpec(specs.effects, name, { name, value: eff.type, isDS: isDS(name), layerName: n.name }, n.id);
            });
          }
        }
    ...
    figma.ui.postMessage({
      ...
      data: {
        ...
        spacings: formatMap(specs.spacings),
  ...
  if (msg.type === "get-selection-link") {
    ...
    if (selection.length > 0) {
      ...
      figma.notify("Selecione um elemento no Figma primeiro para vincular.");
    }
  }
  ...
  if (msg.type === "create-unified-spec") {
    (async () => {
      ...
      specGroup.locked = true; // Bloquear para não atrapalhar a seleção
