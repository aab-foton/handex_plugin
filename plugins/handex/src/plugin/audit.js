...
export function isDS(name, value, type, figmaKey, referenceTokensInput, isAudit, frameJson, nodeName) {
  ...
  // 1. Check if it's an official [dsc] component/element (REGRAS_DSC: Oculto temporariamente)
  ...
  if (isAudit && referenceTokensInput) {
    ...
    for (const referenceTokens of referenceList) {
      ...
      // Global soft match in this reference
    ...
    // Naming conventions / heuristic warnings (applied globally if no exact match found)
  ...
  // If it's an official element but property is not linked, it's a warning by default (REGRAS_DSC: Oculto temporariamente)
