...
document.getElementById('btn-map-pages')!.onclick = () => {
...
function updateScanButton() {
  const isPagesView = !viewPages.classList.contains('hidden');
  const checked = pagesListContainer.querySelectorAll('input[type="checkbox"]:checked').length;
  if (isPagesView && checked > 0) {
    btnStartScan.classList.remove('hidden');
    btnStartScan.classList.add('flex');
  } else {
    btnStartScan.classList.add('hidden');
    btnStartScan.classList.remove('flex');
  }
}
...
onmessage = async (event) => {
  ...
  if (msg.type === 'scan-complete') {
    ...
    if (projectData.designTokens && projectData.designTokens.raw.length > 0) {
        finalAiTextDump += `\n\n=== DESIGN TOKENS (SISTEMA DE DESIGN) ===\n`;
        const tokensByCollection: Record<string, any[]> = {};
        projectData.designTokens.raw.forEach((t: any) => {
            if (!tokensByCollection[t.collectionName]) tokensByCollection[t.collectionName] = [];
            tokensByCollection[t.collectionName].push(t);
        });

        Object.keys(tokensByCollection).forEach(col => {
            finalAiTextDump += `\nColeção: ${col}\n`;
            // Limit output if too many tokens
            const subset = tokensByCollection[col].slice(0, 50);
            subset.forEach(t => {
                const modes = Object.values(t.values).join(' | ');
                finalAiTextDump += `  - ${t.name} [${t.type}]: ${modes}\n`;
            });
            if (tokensByCollection[col].length > 50) {
                finalAiTextDump += `  ... (+${tokensByCollection[col].length - 50} tokens)\n`;
            }
        });
    }
    ...
    if (projectData.libsUsed.length > 0) {
      const libsList = projectData.libsUsed.map((l: string) => `<div class="flex items-center gap-2 text-xs py-1 border-b border-zinc-50 dark:border-zinc-800 last:border-0"><svg class="w-3 h-3 text-green-500" fill="currentColor" viewBox="0 0 20 20"><path d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"></path></svg>${l}</div>`).join('');
      html += createAccordion('Bibliotecas de Design', libsList);
    }
  ...
  if (msg.type === 'export-complete') {
    ...
    await exportToZip(projectData, msg.exportData, finalAiTextDump, attachedFiles, repoType);
