...
> Status (atualizado 2026-07-09): Propostas 1 e 3 **implementadas** em `renderSpecsList()` (`specifications.js`), a função REAL que alimenta `#specs-results` (view "Anotar Specs"). O desenho original abaixo cita `toggleSpecGroupVisibility(frameId, letter)`/`renderSpecsListForFrame` como base de reaproveitamento — essa função é **órfã** (nunca é chamada de fato porque `document.getElementById('specs-list-' + frameId)` nunca existe no HTML) e opera sobre `frame.createdSpecs` (specs por frame), um sistema paralelo e não relacionado ao array global `createdSpecs`/`handoffData.specs` que `renderSpecsList()` de fato renderiza. A implementação real não tem `frameId` disponível (specs globais não são escopadas por frame) — o agrupamento por tag usa apenas `spec.letter`, e o estado de "linhas ocultas" foi persistido em `handoffData.specLinesVisible[letter]` (mapa global, análogo ao `frame.specLinesVisible` da função órfã, mas sem escopo de frame). O cadeado por grupo foi implementado como cadeado **por spec individual** (`spec.locked`, botão ao lado do olho/lixeira em cada item), não por grupo — decisão tomada porque cada spec já tem seu próprio `spec.id` de nó no canvas e o handler `unlock-spec-group` aceita `specIds` de qualquer tamanho. Proposta 2 (ordenação de camadas) segue não implementada.
