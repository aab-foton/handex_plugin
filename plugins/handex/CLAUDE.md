...
- **Wrapper `.[base] *` (INSTANCE) auditado isoladamente mesmo com filho DS real** — a proteção `_hasDSChild` (não auditar container que tem filho DS real, pular pra o filho) só se aplicava a `node.type !== 'INSTANCE'`, nunca a instâncias — a premissa era "instância real do DSC pode legitimamente conter sub-instâncias e ainda ser o item correto a auditar". Isso vale para `[dsc] *`, mas não para o prefixo `.[base] *`: convenção das libs DSC para peça de composição interna (wrapper/slot holder), nunca publicada/consumida isoladamente. Caso real (2026-09-04): `.[base] Menu logo` (INSTANCE, dentro de `[dsc] Menu Lateral` > `.[base] Menu background`) contém `[dsc] Slot` como filho real, mas era auditado sozinho contra o skeleton — nunca bate porque `.[base] Menu logo` não é publicado independente — resultado "FORA DO PADRÃO" com o vínculo DSC vivo bem ali no filho. Corrigido em v6.8.4: instâncias com nome `/^\.\[base\]/i` também são tratadas como wrapper estrutural e puladas quando têm filho DS real, igual a COMPONENT/COMPONENT_SET.
