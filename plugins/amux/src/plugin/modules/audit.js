...
const AMUX_ARTEFATO_FRAMEWORK_MAP = {
...
// canvas (ver AMUX_ARTEFATO_FRAMEWORK_MAP), avisa o usuário — não impede
...
function _renderFrameworkSuggestion(etapa) {
  ...
  const fwId = AMUX_ARTEFATO_FRAMEWORK_MAP[tipo];
  const fw = fwId && (typeof AMUX_FRAMEWORKS !== 'undefined' ? AMUX_FRAMEWORKS : []).find(f => f.id === fwId);
  if (!fw) {
