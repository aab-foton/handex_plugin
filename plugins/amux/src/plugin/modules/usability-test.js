...
// coletar e analisar sessões de teste (AMUX)
...
function _findTest(id) {
  return (amuxData.usabilityTests || []).find(t => t.id === id);
}
...
function createUsabilityTest() {
  ...
  amuxData.usabilityTests.push(test);
  saveState();
  updateHomeBadges();
  closeNewUsabilityTestModal();
...
function confirmDeleteUsabilityTest() {
  ...
  amuxData.usabilityTests = (amuxData.usabilityTests || []).filter(t => t.id !== id);
  saveState();
  updateHomeBadges();
...
// converte para sessões do AMUX, evitando digitação manual de um relatório
...
// (Screen ID) não são importados — exigiriam os "passos" da tarefa AMUX
