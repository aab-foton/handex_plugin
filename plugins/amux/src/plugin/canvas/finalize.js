...
async function finalizeFrame(mainFrame, framework, ts) {
  ...
  // existam groups duplicados/renomeados ou frames [AMUX] órfãos
  ...
  // Frames [AMUX] que já estão soltos como filhos diretos da página
  ...
  const sameFrameworkCount = priorFrames.filter(n =>
    n.getSharedPluginData('maturai', 'frameworkId') === framework.id
  ).length;
  ...
  mainFrame.setSharedPluginData('maturai', 'version', String(version));
  ...
  return {
    ...
    injectedAt: mainFrame.getSharedPluginData('maturai', 'injectedAt'),
    version,
...
module.exports = { finalizeFrame, AMUX_GROUP_NAME };
