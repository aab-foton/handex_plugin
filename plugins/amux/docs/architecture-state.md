...
- Storage key: `amux-data` (clientStorage). Canvas prefix: `[AMUX]`.
...
- `injectFramework()` / `scanFrameworks()`: herdados do Maturai UX, intactos. **Atenção**: o namespace do `setSharedPluginData` continua `'maturai'` (não renomeado) para não quebrar o scan de frames já criados por usuários do plugin antigo.
